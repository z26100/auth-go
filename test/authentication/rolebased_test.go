...
func TestGetPublicResourceWithoutAuthentication(t *testing.T) {
	...
	claimHandler := auth.DefaultRoleBasedClaimHandler(auth.NewSimpleRoleAuthorization())
...
func TestGetResource(t *testing.T) {
	...
	strip := http.StripPrefix("/api/v2", handler)
	// public endpoints can be viewed by everybody
	r, err := http.NewRequest("GET", "https://example.com/api/v2/test", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetAdminResource(t *testing.T) {
	provider := auth.NewFilePublicKeyProvider("sample-pk.pem")
	verifier, err := auth.NewJwtVerifier(provider)
	if err != nil {
		t.Fatal(err)
